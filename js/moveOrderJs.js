...
$("button[name='move_item']").click(function (e) {
  ...
  tbody.forEach((tr) => {
    ...
    if (checkbox.checked) {
      ...
      move_item.transaction_id = tr
        .querySelector("input[name='item_code']")
        .getAttribute("transaction-id");

      console.log(move_item);

      //data object from last to next transaction
      move_item.destination_sub_inventory = parseInt(
        tr.querySelector("select[name='destination_sub_inventory']").value
      );
      move_item.source_sub_invetory = parseInt(
        document.querySelector("select[name='source_sub_invetory']").value
      );
