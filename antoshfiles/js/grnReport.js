function addRowToTable(data) {
  ...
  data.forEach((row, index) => {
    ...
    tr.innerHTML = ` 
                            ...
                            <td class="px-4 py-3">${row.recQty}</td>

                            <td class="px-4 py-3">${row.createdBy}</td>
                            <td class="px-4 py-3">${row.createdDate}</td>
                            <td class="px-4 py-3">${row.po_line_id}</td>
...
$(document).ready(function () {
  ...
  <div class="ground">
    ...
    $.getJSON("./phpAjax/grnAjax.php", data, function (data) {
      console.log(data);
